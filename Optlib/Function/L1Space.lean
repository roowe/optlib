...
- `l1_norm_eq` : Assume {bᵢ} is a basis for finite dimensional space α,
  ∀ x ∈ α, x = ∑ aᵢbᵢ, then we get l₁ norm by map σ which is
  ‖σ(x)‖ = ∑ ‖aᵢ‖ * ‖bᵢ‖
-/
...
noncomputable def f :
    Fin (Module.finrank ℝ α) → PiLp 1 (fun _ : Fin (Module.finrank ℝ α) => ℝ) :=
  fun i => WithLp.toLp 1 (fun j => if i = j then ‖b i‖ else 0)

noncomputable def σ := Module.Basis.constrL b f

theorem continuous_map_sigma : Continuous (σ (α := α)) := by
  exact ContinuousLinearMap.continuous σ
...
theorem sigma_apply_basis (i : Fin (Module.finrank ℝ α)) :
    σ (b i) = fun j => if i = j then ‖b i‖ else 0 := by
  ext j
  simp [σ, f]

theorem sigma_norm_apply : ∀ x, ∀ j, ∑ i, (((b).repr x) i) • σ (b i) j
    = (((b).repr x) j) * ‖b j‖ := by
  intro x j
  simp [sigma_apply_basis, smul_eq_mul]

theorem sigma_decompose_apply : ∀ x, ∀ j, (σ x) j =
    ∑ i, (((b).repr x) i) • σ (b i) j := by
  intro x j
...
in the finite basis representation of x, each multiplied by the norm of the corresponding
basis vector.
-/
theorem l1_norm_eq : ∀ x, ‖σ x‖ = ∑ i, ‖((b).repr x) i‖ * ‖b i‖ := by
  intro x
...
-/
theorem l1Ball_sub_convexHull {x : α} {r : ℝ} (hr : r > 0)
    (hn : Module.finrank ℝ α ≠ 0) :
  ...
  have lem_i {i : ℕ} (hi : i < n + 2) (h₁ : ¬i = n + 1) (h₂ : ¬i = n) :
      i < n := by
    by_contra h₃
    push Not at h₁ h₂ h₃
  ...
  let z : ι₀ → α
    | ⟨i, hi⟩ =>
      if h₁ : i = n + 1 then - (r / ‖b fin0‖) • (b fin0)
        ...
        ((SignType.sign ((b).repr (x₀ - x) ⟨i, lem_i hi h₁ h₂⟩)) *
            (r / ‖b ⟨i, lem_i hi h₁ h₂⟩‖)) •
          (b ⟨i, lem_i hi h₁ h₂⟩)
  ...
  have hz :
    ...
    · apply Set.mem_union_right
      exact Set.mem_iUnion.mpr ⟨fin0, by simp [h₁]⟩
    simp only [h₁]
    by_cases h₂ : i = n
    ...
    · apply Set.mem_union_right
      ...
      refine ⟨use_i, ?_⟩
      have hcond :
          ((b).repr x₀) use_i - ((b).repr x) use_i ≠ 0 := by
        simpa [a, map_sub] using ne_of_lt ha
      rw [sign_neg ha]
      simp [hcond, use_i]
    · apply Set.mem_union_left
      apply Set.mem_iUnion.mpr
      ...
      have hcond :
          ((b).repr x₀) use_i - ((b).repr x) use_i = 0 := by
        simpa [a, map_sub] using ha
      have hzero : (b).repr (x₀ - x) use_i = 0 := by
        simpa [a] using ha
      rw [hzero, sign_zero]
      simp [use_i]
    · apply Set.mem_union_left
      ...
      have hcond :
          ((b).repr x₀) use_i - ((b).repr x) use_i ≠ 0 := by
        simpa [a, map_sub] using ne_of_gt ha
      rw [sign_pos ha]
      simp [hcond, use_i]
  ...
  have hx : ∑ i : ι₀, w i • z i = x₀ - x := by
    ...
    have : w₁ i • (SignType.sign ((b).repr (x₀ - x) i) * (r / ‖b i‖)) = w₀ i := by
      ...
      calc
        _ = |(b).repr (x₀ - x) i| * (‖b i‖ / r) *
            SignType.sign ((b).repr (x₀ - x) i) * (r / ‖b i‖) := by
          rw [← mul_div]
          linarith
        _ =
            (SignType.sign ((b).repr (x₀ - x) i) * |(b).repr (x₀ - x) i|) *
              ((‖b i‖ / r) * (r / ‖b i‖)) := by
          linarith
        _ = _ := by
          rw [sign_mul_abs]
